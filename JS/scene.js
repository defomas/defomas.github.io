...
//                scene.add(cameraHelper);
        ...
        loader.load('../models/Bambo_House.json', function (obj) {
            ...
            scene.add(obj);
            // getting center of the house
            houseObj = obj.getObjectByName("body_house");
            // adding meshes to scene array
        ...
        lightFolder.add(guiControls, 'shadowCameraNear', 0, 100).name("Near").onChange(function (value) {
            spotLight.shadow.camera.near = value;
            spotLight.shadow.camera.updateProjectionMatrix();
            cameraHelper.update();
        });
        lightFolder.add(guiControls, 'shadowCameraFar', 0, 5000).name("Far").onChange(function (value) {
            spotLight.shadow.camera.far = value;
            spotLight.shadow.camera.updateProjectionMatrix();
            cameraHelper.update();
        });
        lightFolder.add(guiControls, 'shadowCameraFov', 1, 180).name("Fov").onChange(function (value) {
        ...
        // load video must call after setting/changing source
        video.load(); 
